import { E as ENABLE_WAL_SQL, C as CREATE_TABLES_SQL } from './chunk-ATP4SNLM.mjs';
import { v4 } from 'uuid';
...
var NodeSqliteStorage = class {
  ...
  async getDailyStats(days = 7) {
    ...
    return rows.map((r) => {
      ...
      return {
        date: String(r.date),
        cost: Number((_a = r.cost) != null ? _a : 0),
        saved: Number((_b = r.saved) != null ? _b : 0),
        tokens: Number((_c = r.tokens) != null ? _c : 0)
      };
    });
  }
  async recordToolCall(call) {
    var _a, _b, _c;
    const id = (_a = call.id) != null ? _a : v4();
    this._db.prepare(
      ...
      id,
      call.session_id,
      (_b = call.event_id) != null ? _b : null,
      call.tool_name,
      (_c = call.target) != null ? _c : null,
      call.tokens_used,
      call.cost_usd,
      call.timestamp
    );
  ...
  async getRecentToolCalls(session_id, limit = 5) {
    const rows = this._db.prepare("SELECT * FROM tool_calls WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?").all(session_id, limit);
    return rows.map((r) => {
      var _a, _b;
      return {
        id: String(r.id),
        ...
        tokens_used: Number((_a = r.tokens_used) != null ? _a : 0),
        cost_usd: Number((_b = r.cost_usd) != null ? _b : 0),
...
//# sourceMappingURL=node-sqlite-CGRNBAZP.mjs.map
