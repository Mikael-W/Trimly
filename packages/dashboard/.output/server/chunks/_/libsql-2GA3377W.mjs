import { E as ENABLE_WAL_SQL, C as CREATE_TABLES_SQL } from './chunk-2FGFBRFQ.mjs';
...
var LibsqlStorage = class {
  ...
  async getDailyStats(days = 7) {
    ...
    return result.rows.map((r) => {
      ...
      return {
        ...
        cost: Number((_a = row["cost"]) != null ? _a : 0),
        saved: Number((_b = row["saved"]) != null ? _b : 0),
        tokens: Number((_c = row["tokens"]) != null ? _c : 0)
      };
    });
  }
  async recordToolCall(call) {
    ...
    await this.exec(
      ...
      VALUES (?,?,?,?,?,?,?,?)`,
      [id, call.session_id, (_b = call.event_id) != null ? _b : null, call.tool_name, (_c = call.target) != null ? _c : null, call.tokens_used, call.cost_usd, call.timestamp]
    );
    return id;
  }
  async getRecentToolCalls(session_id, limit = 5) {
    ...
    return result.rows.map((r) => {
      ...
      return {
        ...
        tokens_used: Number((_a = row["tokens_used"]) != null ? _a : 0),
        cost_usd: Number((_b = row["cost_usd"]) != null ? _b : 0),
        timestamp: Number(row["timestamp"])
      };
    });
  }
...
//# sourceMappingURL=libsql-2GA3377W.mjs.map
